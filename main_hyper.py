...
def adversarial_objective(hyperparams):
    global eval_num
    global best_loss
    global best_hyperparams

    eval_num += 1
    (learning_rate_dis, momentum_dis, multiplier_lr, multiplier_mom, k) = hyperparams
    #use multiplier to ascertain generator does not learn too quickly and cause divergence
    learning_rate_gen = multiplier_lr*learning_rate_dis
    momentum_gen = multiplier_mom*momentum_dis
    print(hyperparams)

    l, pred1, fw1, bw1, tl1, tr1 = adversarial.main(FLAGS.hyper_train_path, FLAGS.hyper_eval_path, accuracyOnt, test_size, remaining_size, learning_rate_dis,
                                                       learning_rate_gen,0.3, momentum_dis, momentum_gen, 0.00001, k,False)
    tf.reset_default_graph()
